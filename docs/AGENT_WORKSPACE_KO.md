...
6. deterministic `use_cache=False` decode 뒤 반복, 역할 토큰, 거짓 정체성, 미완성
   문장을 검사한다.
...
첫 명령은 네 번의 실제 오프라인 대화를 검사한다. 이는 20턴 독립 corpus나 RTX
4090 인증을 대신하지 않는다. 최신 scoped GPU 관측과 정확한 제한은
