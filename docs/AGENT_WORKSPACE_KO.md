...
추가 로컬 snapshot을 목록에 표시하려면 전용 absolute registry를 명시한다. registry는
직접 자식 디렉터리와 같은 이름의 sibling manifest를 한 쌍으로 둔다.

```text
C:\Project\cognios\registry\trusted-e4b\
C:\Project\cognios\registry\trusted-e4b.manifest.toml
```

```powershell
$env:COGNI_OS_MODEL_REGISTRY_DIR = 'C:\Project\cognios\registry'
```

검색은 registry의 직접 자식만 최대 16개(전체 entry 64개) 확인한다. 상대·UNC·URL·Hub
ID, symlink/junction/reparse point, 탈출 경로, 128 KiB 초과 manifest, digest 불일치,
closed-world inventory 불일치, 현재 source에 고정된 instruction-tuned E4B fingerprint가
아닌 candidate는 모두 거부한다. 거부 이유는 host 경로 없이 capability payload에
bounded code로 표시된다. 발견은 로딩 권한이 아니다. 현재 worker만 선택 가능하며,
다른 검증 candidate는 lease drain→unload→memory check→load→rollback 전환이 별도로
구현·검증될 때까지 UI에서 비활성화된다.
...
4. 답변에 사용된 출처는 attachment id, chunk index, score와 함께 표시된다. 현재
   256차원 결정론적 lexical vectorizer는 학습된 semantic embedder가 아니다. 별도
   CPU semantic 검증/테스트 경계는 artifact·품질·라이선스 검증 전이고, production
...
일반 웹 검색은 Lens와 별도이며 `api.search.brave.com` 공식 JSON API만 사용한다.
운영자 online/provider/host/token/terms gate를 모두 설정한 뒤에도 사용자가 검색마다
one-shot 외부 호출 동의를 체크해야 한다.
...
redirect·HTML scraping·결과 URL 자동 fetch는 차단된다. 현재 mock 회귀는 통과했지만
승인 subscription의 live 응답과 packet/egress 감사를 완료로 주장하지 않는다.
