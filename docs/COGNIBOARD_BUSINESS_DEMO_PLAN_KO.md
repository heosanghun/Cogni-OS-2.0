...
| Peak VRAM | p.2는 장비 적합성만 주장 | UI의 14.8560 GiB는 integrated inference peak이고, 문서상 process-wide model-load peak는 14.8955 GiB다. | 측정 구간을 이름에 포함하고 process 최대값도 함께 공개 | CUDA allocated/reserved 시계열과 측정 구간 | 실측 |
...
| 자율 진화 | 등록특허와 야간 자율학습(p.2) | 현재 production patch 실행에는 kernel-isolated sandbox가 없고 기본 경로는 fail-closed | "정책·digest·sandbox·회귀시험을 모두 통과한 경우에만 승격하는 연구 제어면" | 실제 격리 runner 도입 전 실행 금지 | 검증/계획 |
...
- `INITIAL_METRICS`의 14.8560 GiB, depth 100, 178 tests 등은 **내부 실측
  스냅샷**이다. 새 라이브 실행 전에는 `LIVE`가 아니라 `SNAPSHOT`으로 읽혀야 한다.
- live worker의 terminal metrics에는 test count가 없다. 따라서 실행 후에도 남는
  `178 tests`는 현재 run의 결과가 아니라 release 검증 스냅샷이다.
- UI의 `Network BLOCKED`는 offline 환경변수와 구성 정책의 증거다. 별도 packet
  capture 또는 firewall audit가 없다면 "egress 0 실측"으로 표시하지 않는다.
- UI의 14.8560 GiB에는 `integrated inference peak`라는 측정 구간을 붙인다.
  process-wide maximum 14.8955 GiB도 Fact-book에서 함께 공개한다.
...
| 항목 | 등급 | 주의사항 |
|---|---|---|
| RTX 5090 integrated peak 14.8560 GiB | 실측 스냅샷 | integrated inference 구간 명시 |
| process-wide max 14.8955 GiB | 실측 스냅샷 | model-load 구간 포함, cherry-pick 방지 |
...
| 회귀 테스트 178개 | 검증 스냅샷 | live worker result가 아니며 commit을 붙여야 함 |
...
- [ ] 14.8560과 14.8955의 측정 구간을 각각 명시
