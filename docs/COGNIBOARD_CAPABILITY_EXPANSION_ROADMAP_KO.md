...
| 첨부·RAG | content-addressed blob, 영속 catalog, PDF 추출, page/offset/digest provenance, 악성 corpus, 삭제·재색인·restart rebuild | 검증된 semantic embedder와 exact-scope attestation 미완료 |
...
| Self-Harness | 기본 UI의 증거 ledger·read-only diff와 내부 Linux ATTESTED 평가·외부 서명 승격·signed committed rollback 경로 | 자동 승격·UI apply endpoint·production isolation attestation·전체 E2E 없음 |
...
| W5 | 모델 선택기 | 부분 구현 | bounded absolute registry에서 검증된 복수 E4B candidate를 자동 발견·표시하지만 현재 worker만 선택 가능 | 발견 모델의 lease-safe unload/load, GPU/modalities 전환과 rollback 검증 |
...
| W8 | 코드 작업 환경 | 부분 구현 | bounded read/search/fixed pytest, safe output bundle, read-only UI와 내부 ATTESTED 평가·외부 서명 승격·signed committed rollback | hostile-code production isolation 독립 attestation과 전체 operator E2E |
...
signed committed rollback을 수행한다. named engine/image의 CPU integration smoke는
production isolation attestation이 아니며 full E2E도 없으므로 이를 “자가 코드 수정 완료”라고
표시해서는 안 된다.
...
7. 내부 승인·OCI test·apply·signed committed rollback 경로를 hostile-code production
   isolation의 독립 attestation 및 한 환경의 전체 E2E와 결합한다. 그 전까지 사용자 UI와
