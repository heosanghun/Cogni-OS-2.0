# CogniBoard v0.4.0 기능 확장 로드맵

> 이 문서는 v0.4.0 통합 **작업 트리**의 현재 구조와 남은 검증을 정리한다.
...
| 시스템 설계 | Cogni-Core/Cogni-Flow, 주·야간, 입력·검색 흐름 | 실행 권한과 데이터 흐름의 차이를 계속 명시해야 함 | v0.4.0 아키텍처와 UI capability 상태 동기화 | P1 |
