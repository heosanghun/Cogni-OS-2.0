...
| 입력 도구 | 화면에서 확인할 상태 | 현재 의미 |
|---|---|---|
| 파일·이미지 | `로컬` 또는 `사용 불가` | 로컬 admission과 무결성 검사를 통과한 파일만 받는다. 이미지·PDF 칩의 `로컬 저장·모델 미전달`은 현재 멀티모달 추론에 쓰이지 않는다는 뜻이다. |
| 로컬 RAG | `문서 필요`, `사용 가능`, `사용 중`, `미연결` | 사용자 지정 AkasicDB의 고정 커밋 adapter 상태다. 텍스트 문서가 색인된 뒤 대화 모드에서만 켤 수 있다. |
| 웹 검색 | `오프라인`, `인증 필요`, `허용 준비` | 기본은 차단이다. Lens 자동 검색은 명시적 온라인 모드, 도메인 allowlist와 공식 API 토큰이 모두 필요하다. |
| 로컬 모델 | `검증 모델 고정` | manifest가 검증된 E4B-IT만 선택할 수 있다. 목록에 없는 경로나 원격 모델은 로드하지 않는다. |
| 음성 | `준비 중` 또는 `로컬 ASR` | 체크포인트의 오디오 능력과 실제 마이크/ASR 런타임 연결을 구분한다. |

로컬 E4B-IT 체크포인트에는 이미지와 오디오 processor 설정이 있지만, 현재 텍스트
worker 계약은 `input_ids`와 `attention_mask`만 전달한다. 따라서 멀티모달 입력
adapter와 VRAM/회귀 검증 전에는 파일·이미지 또는 음성 버튼을 모델 기능으로
표시하지 않는다.

로컬 문서 근거를 사용하는 순서:

1. `파일·이미지`에서 UTF-8 `txt`, `md`, `csv`, `json` 문서를 선택한다.
2. admission을 통과한 텍스트 문서는 고정 revision의 AkasicDB adapter에 자동
   색인된다. PDF·이미지는 로컬 저장만 되고 현재 색인·모델 입력에는 쓰이지 않는다.
3. `로컬 RAG`가 `사용 가능`이 되면 눌러 `사용 중`으로 바꾼다.
4. 대화 모드에서 질문한다. 작업 모드로 바꾸면 RAG는 자동 해제된다.
5. 답변의 `로컬 RAG 근거`를 펼쳐 `[근거 N]`, 파일명과 검색 점수를 확인한다.

첨부 한도는 파일당 8MiB, 저장소 누적 32개·64MiB이다. JSON은 파서 안전을 위해
1MiB와 64단계 중첩 한도를 추가로 적용한다. 첨부 카탈로그와 AkasicDB 인덱스는 현재
프로세스 메모리 수명이므로 재실행 후 다시 등록·색인해야 하지만, 로컬 blob은 정리
기능을 구현하기 전까지 디스크에 남으며 누적 quota에 계속 포함된다.
