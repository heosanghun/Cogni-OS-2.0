...
있다. `구성됨·실행 미검증`은 아티팩트와 호출 경로가 있다는 뜻일 뿐이다. 현재
production image/audio processor loader는 검증 바이트 결합 미증명으로 차단되어
있으므로 이미지와 STT 버튼은 비활성 상태가 정상이다. 향후 loader가 활성화되더라도
현재 프로세스의 processor probe와 answer-bearing model inference 증거가 모두
확인되기 전에는 활성화하지 않는다.
...
flowchart LR
    ...
    CORE --> GPU["Cogni-Core · 단일 GPU 소유권"]
    CORE -. "주입 processor 회귀만" .-> IPC["고정 4-텐서 image/audio IPC 계약"]
    IPC -. "production loader 차단" .-> GPU
    ...
    VOICE -. "production STT loader 차단" .-> AGENT
    LOOP --> HARNESS["Self-Harness 제안 ledger"]
    HARNESS --> REVIEW["읽기 전용 unified diff"]
```

자연어는 브라우저와 CPU 제어면에서만 다룬다. 모델 프로세스 경계에는 고정 텐서
스키마, 크기, dtype, deadline, 세션·아티팩트 digest를 검사하는 image/audio 계약이
있지만 현재 회귀는 주입 processor와 fake worker만 사용한다. production loader가
차단되어 있으므로 이미지·audio를 같은 resident Gemma에 실제 전달하는 제품 경로로
해석하지 않는다.
...
| `cogni_core_image` | 미래의 검증된 이미지 한 장 경로에서만 허용할 응답 | 조건부 |
...
현재 v0.4.1 production loader가 차단된 상태에서는 `cogni_core_image` 배지를
게시해서는 안 된다.
...
소스에는 admission close → lease drain → 기존 worker unload 확인 → 주입된 memory-release
proof → 후보 load/health → generation·lease publication fence → CAS commit → 실패 rollback/
safe-mode를 잇는 기본 OFF 제어 primitive가 있다. 그러나 CogniBoard의 실제 resident
supervisor·worker factory·GPU memory probe에는 아직 연결되지 않았다. 따라서 다른 모델을
선택하거나 실제 전환할 수 있다고 안내하지 않는다.
세부 안전 한계는 `docs/MODEL_SWITCH_CONTROL_SAFETY_KO.md`에 정리되어 있다.
...
이미지 입력은 v0.4.1 소스에 고정 스키마 CPU 텐서 IPC와 주입 processor 회귀
경계까지만 구현되어 있다. 현재 production `Gemma4Processor` 생성자는 manifest를
검증한 뒤에도 실패하도록 닫혀 있다. 상위 path 기반 parser가 파일명을 다시 열기
때문에, 검증한 바이트와 실제 parser 입력이 같은 바이트임을 ABA 교체 공격까지
포함해 증명할 수 없기 때문이다. 따라서 현재 배포본에는 이미지 answer authority가
없고 `이미지 사용` 버튼은 비활성 상태가 정상이다.

향후 byte-bound processor loader가 구현된 뒤에도 제품 화면은 다음 조건을 모두
충족한 현재 실행 증거가 있어야만 `이미지 사용` 버튼을 활성화한다.
...
다음 사용 순서는 위 조건을 모두 통과한 미래 릴리스에만 적용한다. 현재 v0.4.1
실행 파일에서 사용할 수 있다는 뜻이 아니다.
...
소스 회귀는 주입 processor로 protocol·텐서 격리·경계 조건을 검사한다. 이는 실제
production processor나 Gemma 이미지 답변을 실행했다는 증거가 아니다. byte-bound
loader, 현재 scope의 answer-bearing inference attestation, 실제 GPU에서 이미지 질문 품질·VRAM·지연
검증이 모두 없으므로 상태는 `production loader 차단`이며, 단순히
`구성됨·실행 미검증`으로 축약해 기능이 곧 사용 가능한 것처럼 표시하지 않는다.

### 6.1 비디오의 현재 범위

비디오는 제품 입력이나 대화 기능으로 연결되지 않았다. 소스에는 외부 decoder가 이미
만든 CPU `uint8` RGB frame 묶음을 받는 전처리 경계만 있다. 이 경계는 frame 수,
해상도, sampling FPS, duration, 누적 pixel/byte, 입력·출력 tensor element를 제한하고
고정 Gemma 4 E4B-it trust root를 확인한다. 파일 경로, URL, encoded container, decoder
프로세스, protocol v4 IPC, model forward는 받지 않는다. 이 경계의 현재 회귀는 주입된
processor를 사용하며, production `Gemma4Processor` path-loader도 검증 바이트 결합 미증명으로
비활성이다. 동기 processor 내부의 실제 일시 CPU 할당과 wall-time도 완전히 강제하지
못하므로 상태는 `PARTIAL`이다.
...
### 7.2 선택적 semantic encoder 경계

별도의 로컬 semantic artifact verifier와 test-only encoder 경계는 closed-world manifest,
SHA-256 file inventory, 안전한 `safetensors`, 모델/배치/token/RAM 추정 상한을 검사한다.
production Transformers path-loader는 실제 parser 입력과 검증 바이트를 결합할 수 없어
비활성이다. 배포물에는 승인된 embedding artifact가 없고 라이선스는 manifest 선언만
확인하며, 검색 품질·poisoning·latency 독립 검증도 없다. 따라서 capability는
`quality_attested=false`, `answer_bearing=false`, `production_ready=false`이고 현재
AkasicDB 답변 검색은 계속 lexical 경로만 사용한다.
...
### 8.3 일반 웹 검색 — Brave 공식 JSON API

일반 웹 검색은 Lens와 분리된 요청별 opt-in 기능이다. 다음 운영자 gate가 모두 열려도
사용자가 각 요청의 동의 체크를 직접 선택해야 한 번의 검색 권한이 생긴다.

```powershell
$env:COGNI_OS_ONLINE_MODE = '1'
$env:COGNI_OS_GENERAL_WEB_PROVIDER = 'brave'
$env:COGNI_OS_WEB_ALLOWLIST = 'api.search.brave.com'
$env:COGNI_OS_BRAVE_SEARCH_API_TOKEN = '<승인된 Brave Search API token>'
$env:COGNI_OS_BRAVE_SEARCH_TERMS_ACCEPTED = '1'
```

검색 drawer에서 검색어·결과 수를 고르고 `이번 요청의 외부 HTTPS 호출에 명시적으로
동의`를 체크한 뒤 실행한다. 동의는 one-shot lease이며 요청 후 즉시 폐기된다. 취소,
전역 revoke, 앱 종료는 같은 epoch/publication fence에서 결과 게시 권한을 회수한다.
연결은 `api.search.brave.com/res/v1/web/search`의 공개 IP를 검증·고정하면서 TLS SNI와
인증서 hostname을 유지한다. redirect, HTML scraping, 임의 URL, 결과 링크 자동 fetch는
금지된다. 결과에는 조회시각, query SHA-256, source-record SHA-256, 정규화 HTTPS URL이
붙지만 모델 답변이나 RAG 근거로 자동 승격되지 않는다.

현재는 mock transport/API/UI 회귀만 통과했다. 승인 subscription/token의 live response,
검색 품질, rate-limit, 약관, packet/egress 감사 전에는 운영 완료로 표시하지 않는다.
...
v0.4.1 소스에는 별도 원격 STT 없이 동일 manifest-bound Gemma 4 서비스의 audio
입력을 재사용하려는 고정 텐서 계약과 주입 processor·fake worker 회귀가 있다. 그러나
production audio processor loader는 검증 바이트 결합 미증명으로 차단되어 있어 현재
배포본에는 answer-bearing STT 권한이 없다. 마이크 캡처와 WAV 검사는 STT 완료가
아니며 전사 버튼은 비활성 상태가 정상이다.

향후 byte-bound loader가 구현되면 전사는 단일 compute lock 아래에서 실행하고,
대화·라이브 검증·야간 진화 작업과 동시에 GPU를 소유하지 않으며, 결과를 입력창에만
추가해 사용자가 검토·수정한 뒤 전송하게 한다. 그때도 현재 프로세스의 audio
processor probe와 실제 model transcription을 모두 검증하기 전에는
`LOCAL_STT_INFERENCE_UNVERIFIED`로 종료한다. 브라우저의 secure context,
`getUserMedia`, `AudioContext` 지원도 별도로 확인한다.
...
| 선택적 semantic encoder | verifier/test 경계만 부분 구현, production loader 차단 | byte-bound loader·artifact·라이선스·poisoning/검색 품질·RAM/latency evidence 후 RAG 연결 |
| 이미지 한 장 모델 입력 | protocol/주입 processor 경로 구현, production loader 차단 | byte-bound processor·answer-bearing inference attestation·GPU 실측 |
| 비디오 | 주입 processor decoded-frame CPU 전처리만 부분 구현 | byte-bound processor·decoder worker·IPC·실제 Gemma forward·품질/latency/VRAM |
| 일반 웹 검색 | Brave 공식 JSON connector·요청별 opt-in/cancel/revoke 구현·mock 회귀 통과 | 승인 subscription/token live response·브라우저/packet/egress·약관 검증 |
| Lens 공식 API connector | 구현·mock 회귀 테스트됨 | 4개 gate, Lens 승인·토큰·네트워크 |
| 마이크 캡처·WAV·loopback | 구현·회귀 테스트됨 | Windows/브라우저 실제 권한 확인 |
| STT | protocol/주입 processor 경로와 v0.4.0 단일 historical smoke, production loader 차단 | byte-bound processor·current inference attestation·실제 마이크·다화자 WER·소음·지연·packet audit |
...
| 모델 전환 | 기본 OFF 제어 primitive만 부분 구현 | resident supervisor/factory/memory probe 연결·실 GPU E2E |
...
3. 일반 대화, Fact-book, RAG 등 활성 경로와 이미지·STT 같은 차단 경로가 정확히 표시된다.
