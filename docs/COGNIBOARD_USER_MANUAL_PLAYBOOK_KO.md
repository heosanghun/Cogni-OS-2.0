...
배포 번들에 포함된 실제 파일명과 `SHA256SUMS.txt`를 먼저 확인한다. v0.4.0
소스에서 새 번들을 만들기 전에는 이전 버전 EXE를 v0.4.0 실행 파일이라고 부르지
않는다.
...
1. 검증된 배포 폴더의 `CogniBoard-v0.4.0.exe`를 더블클릭한다.
...
| PDF | 별도 격리 worker, RAM 256MiB·CPU 6초·wall 8초, 최대 128페이지·256,000자, 암호화 PDF 차단 |
...
이미지 입력은 v0.4.0 소스에서 Manager → ModelService → 검증된 Gemma4Processor →
...
출처에는 `attachment_id`, `chunk_index`, 점수, 파일명을 표시한다. 답변은 `[근거
N]` 인용이 유효하지 않으면 게시하지 않는다.
...
v0.4.0은 별도 원격 STT를 호출하지 않고 이미 로드된 동일 manifest-bound Gemma 4
...
2026-07-16 개발 장치에서 Microsoft Heami Desktop `ko-KR`로 합성한
...
경로, 기대 동작, 위험, 재현 검사, 롤백 조건, digest를 확인할 수 있다. 이 API와
화면에는 실행, 승인, 적용, source mutation endpoint가 없다. 자동 승격은 차단되어
있다.
...
| 이미지 한 장 모델 입력 | 구현·회귀 테스트됨 | 검증된 processor·production runtime·GPU 실측 |
| Lens 공식 API connector | 구현·mock 회귀 테스트됨 | 4개 gate, Lens 승인·토큰·네트워크 |
| 마이크 캡처·WAV·loopback | 구현·회귀 테스트됨 | Windows/브라우저 실제 권한 확인 |
| STT | 동일 로컬 Gemma 경로 구현·단일 실제 smoke 통과 | 실제 마이크·다화자 WER·소음·지연·packet audit |
| TTS | Windows System.Speech 재생 구현·단일 실제 smoke 통과 | 음성별 품질·접근성·실장치 수용시험 |
