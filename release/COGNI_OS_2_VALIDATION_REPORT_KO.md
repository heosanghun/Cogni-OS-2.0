...
- 릴리스 증거 상태: **HISTORICAL v0.3.0 SCOPE PASS; CURRENT v0.4.1 SCOPE UNVERIFIED**

## 1. v0.3.0 당시 판정 — 현재 v0.4.1 scope에는 재사용 불가
...
v0.3.0 동결 소스의 전체 회귀, 자동화 20턴 완결성 시험, 통합 GPU runtime
실측, System 4 stress 실측은 당시 source/model/config/device scope에서 통과했다.
이 결과는 현재 v0.4.1 작업 소스나 E4B-it/GPU5 scope의 증거가 아니다. 개발 장치
RTX 5090 Laptop GPU의 결과 역시 목표 RTX 4090 인증이 아니다. v0.3.0 release
artifact의 byte 단위 검증값은 당시 `SHA256SUMS.txt`에 기록돼 있다. 당시 원시·구조화
증거는 `release/evidence/`의 20턴 대화, GPU CTS runtime, System 4 stress, 소스 회귀
JSON 4개에 historical comparison으로만 보존한다.
...
## 5. v0.3.0 통합 Gemma CTS V2 GPU 관측 — `HISTORICAL DEV-HOST OBSERVATION; CURRENT SCOPE UNVERIFIED`

현재 v0.4.1 scope의 GPU 재검증은 clean exact-commit CPU Exit Gate를 모두 통과한
뒤에만 수행한다. 유일한 허용 진입점은
[`docs/GEMMA4_VALIDATION.md`](../docs/GEMMA4_VALIDATION.md)에 고정된
`scripts/gpu5_boundary_guard.py` Stage G 경로다. validator나 Docker를 직접 실행하는
구형 재현 명령은 현재 release evidence를 만들 수 없으며 사용하지 않는다.

보존된 v0.3.0 historical 원시 실행 결과는 다음과 같다.
...
이 결과는 v0.3.0 당시 모델·코드·config·개발 장치 scope의 historical `measured`
관측일 뿐, 현재 v0.4.1 scope의 PASS 또는 VERIFIED 증거가 아니다. 개발 장비 RTX
5090 Laptop GPU의 결과는 목표 장치 RTX 4090 인증으로 대체할 수 없다. UI는 시작
시 증거와 수치를 `UNVERIFIED`로 표시하고, 현재 exact commit에서 위 GPU5 guard와
독립 verifier를 모두 통과한 runtime만 live evidence로 승격해야 한다.

## 6. 대화 완결성 검증

아래 수치는 과거 개발 장치 관측이다. 현재 제품 E4B-it completion profile은 전용
GPU5 guard allowlist와 instruction-model snapshot 결합 전까지 `BLOCKED`이며 직접
실행 명령을 배포하지 않는다. 최신 서버 지침은 `docs/VALIDATION.md`만 따른다.
...
이 명령은 CPU 소프트웨어 불변식만 검사한다. System 4 CUDA 측정은 GPU5 guard
allowlist와 실행 전후 identity 계약이 완성될 때까지 `BLOCKED`다.
...
v0.3.0 당시 개발 장치의 historical CUDA 10,000-iteration 관측은 p50 2.9157ms, p95 3.893715ms,
...
historical measurement-only/advisory 실측이며 현재 v0.4.1 scope의 PASS가 아니고,
production PCAS calibration이나 답변 품질
증거로 승격하지 않는다.
...
v0.3.0 당시 소스 트리에서 Ruff check, Ruff format check(126 files), Node syntax
check가 통과했다. 전체 pytest는 599 passed, 3 skipped, 3 deprecation warnings,
482 subtests passed를 97.67초에 완료했다. 이 수치는 historical snapshot이며 현재
v0.4.1 source regression의 PASS 또는 VERIFIED 근거가 아니다. 현재 exact commit의
clean CPU Exit Gate 원시 결과로 다시 검증해야 한다.
...
v0.3.0은 이 의도의 Phase 1~11 소프트웨어·안전 경계를 구현했고, 당시 scope에서
소스 회귀, 자동화 20턴, 통합 GPU runtime, System 4 stress를 통과했다. 이 결과는
현재 v0.4.1 scope에 승계되지 않는다. 당시 최종 artifact의 byte-level 동일성은
해당 `SHA256SUMS.txt`로 확인해야 한다. 또한 자동 승격, 독립 외부
